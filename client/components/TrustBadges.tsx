...
interface TrustBadgeProps {
  variant: 'verified' | 'premium' | 'top-rated' | 'fast-delivery' | 'global' | 'established' | 'power-trader' | 'elite' | 'rising-star';
  className?: string;
  showIcon?: boolean;
  size?: 'sm' | 'md' | 'lg';
}
...
export function UserTrustIndicator({ user, className, maxBadges = 3 }: UserTrustIndicatorProps) {
  const badges: Array<{ variant: TrustBadgeProps['variant']; priority: number }> = [];
  ...
  if (user.rating && user.rating >= 4.8 && user.totalReviews && user.totalReviews >= 50) {
  ...
  if (user.establishedYear && new Date().getFullYear() - user.establishedYear >= 5) {
    badges.push({ variant: 'established', priority: 4 });
  }

  // Special case for new but promising traders
  if (user.totalOrders && user.totalOrders < 50 && user.rating && user.rating >= 4.5) {
...
export function ProductTrustIndicator({ product, className }: ProductTrustIndicatorProps) {
  const badges: Array<{ variant: TrustBadgeProps['variant']; show: boolean }> = [
    ...
    { variant: 'fast-delivery', show: (product.averageDeliveryTime || 0) <= 5 },
  ];
...
export function PlatformTrustSection({ className }: PlatformTrustSectionProps) {
  const trustMetrics = [
    ...
    },
    {
      ...
      description: 'Product quality guaranteed with inspection and certification',
    },
    {
      icon: Globe,
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Our platform is built on trust, security, and transparency to ensure safe and successful international trade.
          </p>
        </div>
