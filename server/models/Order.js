...
orderSchema.virtual('totalPaid').get(function() {
  let total = 0;
  if (this.paymentPlan.advance.status === 'paid') total += this.paymentPlan.advance.amount;
  if (this.paymentPlan.shipment.status === 'paid') total += this.paymentPlan.shipment.amount;
  if (this.paymentPlan.delivery.status === 'paid') total += this.paymentPlan.delivery.amount;
  return total;
});
...
orderSchema.virtual('nextPaymentDue').get(function() {
  ...
  if (this.paymentPlan.shipment.status === 'pending' && this.status === 'shipped') {
    return {
      type: 'shipment',
  ...
  if (this.paymentPlan.delivery.status === 'pending' && this.status === 'delivered') {
    return {
      type: 'delivery',
...
orderSchema.methods.addTimelineEvent = function(event, description, actor, metadata = {}) {
  this.timeline.push({
    event,
    description,
    actor,
...
orderSchema.methods.processPayment = function(paymentType, paymentId, amount) {
  ...
  } else if (paymentType === 'delivery' && this.paymentPlan.advance.status === 'paid' && this.paymentPlan.shipment.status === 'paid') {
...
orderSchema.methods.updateShippingStatus = function(status, trackingNumber, estimatedDelivery) {
  this.status = status;
